...
const components = {
  ...
  pre: ({ className, ...props }: ComponentsProps) => (
    <pre
      className={cn(
        "mb-4 mt-6 overflow-x-auto text-sm rounded-lg border !bg-zinc-900 py-4 text-gray-100",
        className
      )}
      {...props}
    />
  ...
  code: ({ className, ...props }: ComponentsProps) => (
    <code
      className={cn(
        "relative rounded border px-[0.3rem] py-[0.2rem] !bg-zinc-900 font-code font-light !text-sm text-gray-100",
        className
      )}
      {...props}
    />
